...
*   **O Problema da Busca Semântica Assimétrica (Resolvido com HyDE):** Percebemos que o RAG falhava ao relacionar sintomas genéricos (ex: "febre, náusea") com as doenças corretas. Isso ocorria porque a base de dados (MedQuAD) mapeia *Doença -> Sintomas*, enquanto o médico mapeia *Sintomas -> Doença*. A solução foi implementar a técnica de **HyDE (Hypothetical Document Embeddings)** via LangChain: um passo intermediário oculto onde o modelo prevê possíveis diagnósticos a partir dos sintomas, expandindo a busca no ChromaDB. 
*   **Gestão de Contexto em Múltiplos Turnos (Conversational RAG):** Para garantir que perguntas de *follow-up* (ex: "Qual o tratamento?") não quebrassem o fluxo, implementamos uma cadeia de **Standalone Question Generation**. Em vez de reutilizar as hipóteses iniciais do HyDE (o que trazia resultados de múltiplas doenças ao mesmo tempo), o LangChain aciona a LLM para ler o histórico da conversa e reescrever a pergunta atual de forma independente (ex: "Qual o tratamento para Encefalomielite Disseminada Aguda?"). Isso transformou a precisão do retrieval, trazendo exatamente a literatura que o médico precisava e mantendo a coerência cirúrgica na conversa.
